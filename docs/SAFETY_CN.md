...
- 非有限 JSON/CBOR 浮点会报错；
- 文本解码器（YAML、TOML、RON、Hjson、S 表达式、CSV、urlform）拒绝非有限浮点，
  包括 `1e999` 这类会被 `str::parse::<f64>()` 接受为无穷的溢出拼写；
- MessagePack 是唯一的有意例外：其线格式可无损表示 NaN/无穷，非有限浮点在线上
  透传（符合格式自身语义），仅在中继到无法表示它们的格式时报错；类型化 `f64`
  解码拒绝它们；
- 解码器默认拒绝超过 128 层的嵌套。
