...
//!
//! The nextjson dependency enables the `simd` feature, so the numbers here
//! reflect the architecture-accelerated scanning paths (SSE2/AVX2 on x86-64,
//! NEON on aarch64) that the GitHub Actions benchmark workflow reports.
//!
//! The same fixture, the same warm-up, and the same measurement loop are used
//! for every library, on the same machine and process. Output is CSV with a
//! single header row:
//!
//! ```text
...
//!
//! Honest caveats:
//! - `simd-json`'s serde decoder requires a mutable input buffer (it parses
//!   in place), so every decode iteration pays a `to_vec()` copy of the wire
//!   bytes. That copy is part of the real cost of using its serde API and is
//!   included in the measurement.
//! - `bincode` has no nextjson counterpart (nextjson has no bincode codec);
//!   a `nextjson_bincode(na)` row is emitted for symmetry.
//! - TOML/BSON are document-shaped and postcard rejects signed scalars, so
//!   those three are measured on a `Config` fixture instead of `Vec<Record>`.
...
/// A string-heavy fixture: long realistic text bodies. This is the workload
/// where register-width / SIMD string scanning matters most, so it directly
/// measures the acceleration in the nextjson `simd` feature.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, NsonSerialize, NsonDeserialize)]
struct LongText {
    id: u64,
    title: String,
...
/// A ~1.5 KiB paragraph of realistic prose. It contains a few quotes and
/// backslashes at the very end (so the long clean prefix exercises the
/// chunked/SIMD scan) and one escaped tab.
...
fn longtext_fixture() -> Vec<LongText> {
    (0..32)
        .map(|index| LongText {
            id: index,
...
/// `decode` consumes them; both are dyn calls so the compiler cannot elide
/// them.
...
/// simd-json's serde decoder parses in place and therefore requires `&mut
/// [u8]`; each decode iteration copies the wire bytes once. The copy is part
/// of the real cost of the library's serde API and is included in the
/// measurement.
...
fn main() {
    ...
    let longtexts = longtext_fixture();
    ...
    check_serde(
        &config,
        |b| toml::from_str(std::str::from_utf8(b).unwrap()),
    ...
    check_serde(
        &config,
        |b| postcard::from_bytes(b),
        || postcard::to_allocvec(&config).unwrap(),
    );
    check_nextjson(&longtexts, nextjson::formats::Json);
    check_serde(
        &longtexts,
        |b| serde_json::from_slice(b),
        || serde_json::to_vec(&longtexts).unwrap(),
    );
    check_serde(
        &longtexts,
        |b| Ok::<Vec<LongText>, simd_json::Error>(simd_json_serde_decode(b)),
        || simd_json::serde::to_vec(&longtexts).unwrap(),
    );

    println!("case,size_bytes,encode_ops,encode_MBps,decode_ops,decode_MBps");

    // ---- JSON (nextjson / serde_json / simd-json) -------------------------
    pair_bench(
        "nextjson_json",
        "serde_json",
        duration,
    ...
    bench(
        ...
        &|| simd_json::serde::to_vec(&records).unwrap(),
        &|b| {
            black_box(simd_json_serde_decode::<Vec<Record>>(b));
        },
    );

    // ---- Long-text JSON (SIMD string scanning workload) -------------------
    pair_bench(
        "nextjson_longtext_json",
        "serde_longtext_json",
        duration,
        &|| nextjson::nextencode(&longtexts).unwrap(),
        &|b| {
            black_box(nextjson::nextdecode::<Vec<LongText>>(b).unwrap());
        },
        &|| serde_json::to_vec(&longtexts).unwrap(),
        &|b| {
            black_box(serde_json::from_slice::<Vec<LongText>>(b).unwrap());
        },
    );
    bench(
        "simd_longtext_json",
        ...
        &|| simd_json::serde::to_vec(&longtexts).unwrap(),
        &|b| {
            black_box(simd_json_serde_decode::<Vec<LongText>>(b));
        },
    );

    // ---- JSON5 (nextjson / serde_json5) -----------------------------------
    pair_bench(
        "nextjson_json5",
        "serde_json5",
        duration,
        &|| nextjson::formats::encode_with(&records, nextjson::formats::Json5).unwrap(),
        &|b| {
            black_box(
                nextjson::formats::decode_with::<Vec<Record>, _>(b, nextjson::formats::Json5)
                    .unwrap(),
            );
        },
        &|| serde_json5::to_string(&records).unwrap().into_bytes(),
        &|b| {
            black_box(
                serde_json5::from_str::<Vec<Record>>(std::str::from_utf8(b).unwrap()).unwrap(),
            );
        },
    );
    ...
    // ---- Float-free control -----------------------------------------------
    // Quantifies how much of the encode gap is float formatting (nextjson
    // uses `core::fmt::Display`; serde_json uses ryu).
    let int_records = int_fixture();
    let njson_json = nextjson::nextencode(&int_records).unwrap();
    assert_eq!(
        nextjson::nextdecode::<Vec<IntRecord>>(&njson_json).unwrap(),
        int_records
    );
    let serde_json_bytes = serde_json::to_vec(&int_records).unwrap();
    let serde_back: Vec<IntRecord> = serde_json::from_slice(&serde_json_bytes).unwrap();
    assert_eq!(serde_back, int_records);
    pair_bench(
        "nextjson_intonly",
        "serde_intonly",
        duration,
        &|| nextjson::nextencode(&int_records).unwrap(),
        &|b| {
            black_box(nextjson::nextdecode::<Vec<IntRecord>>(b).unwrap());
        },
        &|| serde_json::to_vec(&int_records).unwrap(),
        &|b| {
